...
|toAddress |否|string | USDT入金时返回，入金的收款地址  |
|fromAddress |否|string | USDT入金时返回，入金的付款地址  |
|rechargeCurrencyType |否|string | USDT入金时返回，虚拟币类型，例： USDT(TRC20)   |
|hash |否|string | USDT入金时返回，链上此交易的hash地址  |
